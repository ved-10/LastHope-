...
var bg1,bg2,bg3,bg4;
var wallGroup,wall2Group,wall3Group,wall4Group,wsoundGroup,dsoundGroup,msoundGroup,ssoundGroup,wls,wlss,wlsss,wlssss,myfont,enyaGroup,enybGroup
...
plr.addAnimation("stop",plrst)
plr.addAnimation("stopl",plrstl)
...
if(gameState==="Level 2"){
   ...
   // nosound.visible=false
    ...
    wall21.lifetime = 100
     ...
     var enya = createSprite(windowWidth/1.4,windowHeight/1.14,50,100)
     enya.addImage(robol)
     enya.scale = 0.7
     enya.setCollider("rectangle",0,0,100,200)
     var enyhita = createSprite(enya.position.x -200,enya.position.y-22,500,10)
     enyhita.visible = false


     enyaGroup.add(enyhita)



  
     
wlss = [wall10,wall11,wall12,wall13,wall14,wall15,wall16,wall17,wall18,wall19,wall20,wall21,enya,exit2]
...
for(var i = 0;i<13;i++){
  ...
  if(keyDown("e") && keyDown("RIGHT_ARROW") || keyDown("e") && keyDown("d")){
    ...
    stroke("red") 
    strokeWeight(3)
    text("bullets are not affecting the robot,jump to dodge bullets",windowWidth/4,windowHeight/2)
  ...
  if(enyhita.isTouching(plr)){
   ...
   if(bulletGroup.isTouching(enya)){
    ...
    wsoundGroup.add(wall13)
    wsoundGroup.add(wall14)
    wsoundGroup.add(wall15)
    wsoundGroup.add(wall16)
    wsoundGroup.add(wall17)
...
intel2.visible = false
    ...
    enyb.addImage(robol)
    enyb.scale = 0.5
    enyb.setCollider("rectangle",0,-0,30,80)
   var enyhitb = createSprite(enyb.position.x -310,enyb.position.y-22,500,10)
...
for(var i = 0;i<5;i++){
  ...
  if(enyhitb.isTouching(plr)){
    ...
    if(bulletGroup.isTouching(enyb)){
        enyb.x = windowWidth*0
        enyb.y = windowHeight*0
        enyb.visible = false
    }
...
var bullet =createSprite(plr.position.x+20,plr.position.y-35,10,1)
bullet.visible=false
   ...
   if(keyDown("e") && keyDown("RIGHT_ARROW") || keyDown("e") && keyDown("d")){
    ...
    bullet.scale = 0.1
    ss.play()
    bullet.setCollider("rectangle",50,0,30,50)
    }

    if(keyDown("q") && keyDown("LEFT_ARROW") || keyDown("q") && keyDown("a")){
        bullet.visible=true
        bullet.velocityX = -50
        bullet.addImage(bul2Img)
        bullet.scale = 0.1
        ss.play()
        bullet.setCollider("rectangle",50,0,30,50)
        }
...
if(keyDown("LEFT_ARROW")||keyDown("a")){
    ...
    plr.changeAnimation("stopl",plrstl)
    plr.scale=0.95
...
if(keyDown("RIGHT_ARROW")||keyDown("d")){
    ...
    plr.changeAnimation("stop",plrst)
   
}


if(keyDown("UP_ARROW")||keyDown("w") && plr.y <= plr.y+20) {
    plr.velocityY = -18;
}
